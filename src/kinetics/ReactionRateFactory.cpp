...
ReactionRateFactory::ReactionRateFactory()
{
    ...
    // BoltzmannRate evaluator
    reg("Boltzmann", [](const AnyMap& node, const UnitStack& rate_units) {
        return new BoltzmannRate(node, rate_units);
    });

    // PlasmaCustomExpr evaluator
    reg("PlasmaCustomExpr", [](const AnyMap& node, const UnitStack& rate_units) {
        return new PlasmaCustomExpr(node, rate_units);
    });
